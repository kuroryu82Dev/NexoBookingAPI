...
export async function getServices(req, res) {
  ...
  if (category !== undefined && (typeof category !== 'string' || !category.trim())) return jsonError(res, 400, 'El filtro category debe ser una cadena no vacía');
  if (available !== undefined && !['true', 'false'].includes(available)) return jsonError(res, 400, 'El filtro available debe ser true o false');
  try {
    const data = await servicesService.getServices({ category, available: available === undefined ? undefined : available === 'true' });
...
export async function createService(req, res) {
  try { return res.status(201).json({ status: 'success', data: await servicesService.createService(req.body) }); }
